...
static int allgather_spmv(CGSolverData* cg,
                          ...
                          size_t* recvcounts, size_t* displs, cusparseDnVecDescr_t vec_global_descr,
                          cusparseDnVecDescr_t vec_target_local_descr,
                          double* d_spmv_alpha,  // CHANGED: double
                          double* d_spmv_beta,   // CHANGED: double
                          cusparseSpMVAlg_t alg, uniconn::Coordinator<>* coordinator) {
...
int cg_solver_solve(CGSolverData* cg,
    ...
    if (cg->comm_size > 1) {
        ...
        GPU_RT_CALL(
            cudaMemcpyAsync(recvcounts_d, recvcounts, cg->comm_size * sizeof(size_t), cudaMemcpyHostToDevice, stream));
        GPU_RT_CALL(cudaMemcpyAsync(displs_d, displs, cg->comm_size * sizeof(size_t), cudaMemcpyHostToDevice, stream));
    }
...
int main(int argc, char* argv[]) {
    ...
    int device_id;
    GPU_RT_CALL(cudaGetDevice(&device_id));
