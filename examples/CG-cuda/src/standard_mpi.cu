...
{
    // Free device buffers
    GPU_RT_CALL(cudaFree(cg->d_r));
    cg->d_r = NULL;
    GPU_RT_CALL(cudaFree(cg->d_p));
    cg->d_p = NULL;
    GPU_RT_CALL(cudaFree(cg->d_t));
    cg->d_t = NULL;
    GPU_RT_CALL(cudaFree(cg->d_rnrm2sqr));
    cg->d_rnrm2sqr = NULL;
    GPU_RT_CALL(cudaFree(cg->d_pdott));
    cg->d_pdott = NULL;
    ...
    if (cg->matA_descr)
        CHECK_CUSPARSE(cusparseDestroySpMat(cg->matA_descr));
    cg->matA_descr = NULL;
    if (cg->vecr_descr)
        CHECK_CUSPARSE(cusparseDestroyDnVec(cg->vecr_descr));
    cg->vecr_descr = NULL;
    if (cg->vecp_descr)
        CHECK_CUSPARSE(cusparseDestroyDnVec(cg->vecp_descr));
    cg->vecp_descr = NULL;
    if (cg->vect_descr)
        CHECK_CUSPARSE(cusparseDestroyDnVec(cg->vect_descr));
    cg->vect_descr = NULL;
    ...
    cusparseSpMVAlg_t alg)
{
    ...
    if (cg->N_global <= 0)
    {
        if (cg->rank == 0)
            fprintf(
                stderr,
                "Error: cg_solver_init called with N_global=%d\n",
                cg->N_global);
        return 1;
    }
    ...
    if (local_rows > 0)
    {
        ...
        GPU_RT_CALL(cudaMalloc(
            (void**) &cg->d_r, local_rows * sizeof(double))); // CHANGED: double
        GPU_RT_CALL(cudaMalloc(
            (void**) &cg->d_p, local_rows * sizeof(double))); // CHANGED: double
        GPU_RT_CALL(cudaMalloc(
            (void**) &cg->d_t, local_rows * sizeof(double))); // CHANGED: double
        GPU_RT_CALL(
            cudaMemset(cg->d_r, 0, local_rows * sizeof(double))); // CHANGED: double
        GPU_RT_CALL(
            cudaMemset(cg->d_p, 0, local_rows * sizeof(double))); // CHANGED: double
        GPU_RT_CALL(
            cudaMemset(cg->d_t, 0, local_rows * sizeof(double))); // CHANGED: double

        // --- Allocate and copy sparse matrix (local part) to device ---
        GPU_RT_CALL(cudaMalloc(
            (void**) &cg->d_A_rowptr,
            (local_rows + 1) * sizeof(int)));
        GPU_RT_CALL(cudaMemcpy(
            cg->d_A_rowptr, h_A_rowptr,
            (local_rows + 1) * sizeof(int), cudaMemcpyHostToDevice));
        GPU_RT_CALL(cudaMalloc(
            (void**) &cg->d_A_colidx,
            local_nnz * sizeof(int))); // Global indices
        if (local_nnz > 0)
            GPU_RT_CALL(cudaMemcpy(
                cg->d_A_colidx, h_A_colidx, local_nnz * sizeof(int),
        ...
        if (local_nnz > 0)
            GPU_RT_CALL(cudaMemcpy(
                cg->d_A_vals, h_A_vals, local_nnz * sizeof(double), // CHANGED: double
                cudaMemcpyHostToDevice));

        // --- Create cuSPARSE descriptors for matrix and LOCAL vectors ---
        CHECK_CUSPARSE(cusparseCreateCsr(
            &cg->matA_descr, local_rows, cg->N_global, local_nnz,
            cg->d_A_rowptr, cg->d_A_colidx, cg->d_A_vals,
            CUSPARSE_INDEX_32I, CUSPARSE_INDEX_32I,
            CUSPARSE_INDEX_BASE_ZERO, CUDA_R_64F)); // CHANGED: CUDA_R_64F
        CHECK_CUSPARSE(cusparseCreateDnVec(
            &cg->vecr_descr, local_rows, cg->d_r,
            CUDA_R_64F)); // CHANGED: CUDA_R_64F
        CHECK_CUSPARSE(cusparseCreateDnVec(
            &cg->vecp_descr, local_rows, cg->d_p,
            CUDA_R_64F)); // CHANGED: CUDA_R_64F
        CHECK_CUSPARSE(cusparseCreateDnVec(
            &cg->vect_descr, local_rows, cg->d_t,
            CUDA_R_64F)); // CHANGED: CUDA_R_64F
    }
    else
    {
    ...
    GPU_RT_CALL(cudaMemcpy(
        cg->d_one, &one, sizeof(double), cudaMemcpyHostToDevice)); // CHANGED: double
    GPU_RT_CALL(cudaMalloc((void**) &cg->d_minus_one, sizeof(double))); // CHANGED: double
    GPU_RT_CALL(cudaMemcpy(
        cg->d_minus_one, &minus_one, sizeof(double), // CHANGED: double
        cudaMemcpyHostToDevice));
    GPU_RT_CALL(cudaMalloc((void**) &cg->d_zero, sizeof(double))); // CHANGED: double
    GPU_RT_CALL(cudaMemcpy(
        cg->d_zero, &zero, sizeof(double), cudaMemcpyHostToDevice)); // CHANGED: double
    ...
    GPU_RT_CALL(
        cudaMalloc((void**) &cg->d_rnrm2sqr_prev, sizeof(double))); // CHANGED: double
    ...
    cusparseDnVecDescr_t tmp_local_target_descr =
        cg->vect_descr;
    double* tmp_d_local_target = cg->d_t; // CHANGED: double

    GPU_RT_CALL(
        cudaMalloc((void**)&tmp_d_global_vec, cg->N_global * sizeof(double))); // CHANGED: double
    CHECK_CUSPARSE(cusparseCreateDnVec(
        &tmp_global_vec_descr, cg->N_global, tmp_d_global_vec,
        CUDA_R_64F)); // CHANGED: CUDA_R_64F
    ...
    {
        CHECK_CUSPARSE(cusparseCreateCsr(
            &tmp_mat_descr, 0, cg->N_global, 0, NULL, NULL, NULL,
            CUSPARSE_INDEX_32I, CUSPARSE_INDEX_32I,
            CUSPARSE_INDEX_BASE_ZERO, CUDA_R_64F)); // CHANGED: CUDA_R_64F
        GPU_RT_CALL(cudaMalloc(
            (void**)&tmp_d_local_target,
            1 * sizeof(double))); // CHANGED: double (Min allocation)
        CHECK_CUSPARSE(cusparseCreateDnVec(
            &tmp_local_target_descr, 0, tmp_d_local_target,
            CUDA_R_64F)); // CHANGED: CUDA_R_64F
    }

    size_t bufferSize = 0;
    CHECK_CUSPARSE(cusparseSpMV_bufferSize(
        cusparse, CUSPARSE_OPERATION_NON_TRANSPOSE, cg->d_one,
        tmp_mat_descr, tmp_global_vec_descr, cg->d_zero,
        tmp_local_target_descr, CUDA_R_64F, alg, &bufferSize)); // CHANGED: CUDA_R_64F
...
void mpi_allgatherv(double* sendbuf, double* recvbuf, int* counts, int* displs, MPI_Comm comm) {
    int comm_size = -1;
    int comm_rank = 0;
    ...
    for (size_t i = 0; i < comm_size; ++i) {
        if (counts[i] > 0) {
            MPI_CALL(MPI_Isend(sendbuf, counts[comm_rank], MPI_DOUBLE, i, 2, comm,
                               &(internal_reqs.emplace_back(MPI_REQUEST_NULL))));
            MPI_CALL(MPI_Irecv((recvbuf + displs[i]), counts[i], MPI_DOUBLE, i, 2, comm,
                               &(internal_reqs.emplace_back(MPI_REQUEST_NULL))));
        }
    }
...
static int allgather_spmv(CGSolverData* cg,
    ...
    // Perform Allgatherv
    if (cg->comm_size > 1) {
        mpi_allgatherv(d_source_local, d_comm_buffer_global, recvcounts, displs, cg->comm);
...
int cg_solver_solve(CGSolverData* cg,
    ...
    if (n > 0) {
        GPU_RT_CALL(cudaMalloc((void**)&d_x, n * sizeof(double)));                         // CHANGED: double
    ...
    if (cg->comm_size > 1) {
        ...
        if (!recvcounts || !displs) {
            ...
            GPU_RT_CALL(cudaFree(d_p_global));
    ...
    MPI_CALL(MPI_Allreduce(MPI_IN_PLACE, d_rnrm2sqr_dev, 1, MPI_DOUBLE, MPI_SUM,  // CHANGED: MPI_DOUBLE
                           cg->comm));
    ...
    */
    ...
    for (k = 0; k < maxits; k++) {
        ...
        GPU_RT_CALL(cudaStreamSynchronize(stream));
        MPI_CALL(MPI_Allreduce(MPI_IN_PLACE, d_pdott_dev, 1, MPI_DOUBLE, MPI_SUM,  // CHANGED: MPI_DOUBLE
                               cg->comm));
        ...
        // double initial_rnorm_sq =
        //     cg->initial_residual_norm * cg->initial_residual_norm;
