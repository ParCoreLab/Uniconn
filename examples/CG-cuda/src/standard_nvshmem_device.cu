...
__global__ void spmv_allgather_fused(int num_rows, int comm_size, int local_rows, const int* recvcount_d,
                                     const int* displs_d, double* d_source_local, double* recvbuf_d,
    ...
    if (n_pes > 1) {
        if (grid.block_rank() == 0) {
            ...
            for (int dest_pe = 0; dest_pe < n_pes; ++dest_pe) {
                nvshmemx_double_put_nbi_block(recvbuf_d + displs_d[my_pe], d_source_local, recvcount_d[my_pe], dest_pe);
            }
            nvshmemx_barrier_all_block();
        }
...
__inline__ __device__ double warpReduceSum(int mask, double x) {
    for (int i = warpSize / 2; i > 0; i /= 2) x += __shfl_down_sync(mask, x, i);
    return x;
}
...
__global__ void ddot_allreduce_fused(int n, const double* __restrict x, const double* __restrict y,
    ...
    if (n_pes > 1) {
        cg::sync(grid);
        if (grid.block_rank() == 0) {
            nvshmemx_double_sum_reduce_block(NVSHMEM_TEAM_WORLD, dot, dot, 1);
        }
    }
}
...
int cg_solver_solve(CGSolverData* cg,
    ...
    if (cg->comm_size > 1) {
        ...
        recvcounts_d = (int*)nvshmem_malloc(cg->comm_size * sizeof(int));
        displs_d = (int*)nvshmem_malloc(cg->comm_size * sizeof(int));
        ...
        GPU_RT_CALL(
            cudaMemcpyAsync(recvcounts_d, recvcounts, cg->comm_size * sizeof(int), cudaMemcpyHostToDevice, stream));
        GPU_RT_CALL(cudaMemcpyAsync(displs_d, displs, cg->comm_size * sizeof(int), cudaMemcpyHostToDevice, stream));
    }
    ...
    if (n > 0) {
        ...
        if (mingridsize == 0 && blocksize == 0) {
            cudaOccupancyMaxPotentialBlockSize(&mingridsize, &blocksize, spmv_allgather_fused, 0, 0);
        }

        void* kernelargs[] = {
            (void*)&N_global, (void*)&cg->comm_size, (void*)&n,          (void*)&recvcounts_d,
        ...
        nvshmemx_collective_launch((void*)spmv_allgather_fused, gridDim, blockDim, kernelargs, sharedmemsize, stream);
        cudaStreamSynchronize(stream);
    }

    // Calculate initial residual norm (all ranks participate)
    if (n == 0) {
        GPU_RT_CALL(cudaMemset(d_rnrm2sqr_dev, 0, sizeof(double)));  // CHANGED: double
    } else {
        static int mingridsize = 0, blocksize = 0, sharedmemsize = 0;
        if (mingridsize == 0 && blocksize == 0) {
            cudaOccupancyMaxPotentialBlockSize(&mingridsize, &blocksize, ddot_allreduce_fused, 0, 0);
        }

        void* kernelargs[] = {(void*)&n, (void*)&d_r, (void*)&d_r, (void*)&d_rnrm2sqr_dev};
        dim3 blockDim(blocksize, 1, 1);
        dim3 gridDim(mingridsize, 1, 1);
        nvshmemx_collective_launch((void*)ddot_allreduce_fused, gridDim, blockDim, kernelargs, sharedmemsize, stream);
        cudaStreamSynchronize(stream);
    }
    ...
    */
    ...
    for (k = 0; k < maxits; k++) {
...

        {
            ...
            dim3 blockDim(blocksize, 1, 1);
            dim3 gridDim(mingridsize, 1, 1);
            nvshmemx_collective_launch((void*)spmv_allgather_fused, gridDim, blockDim, kernelargs, sharedmemsize,
                                       stream);
            cudaStreamSynchronize(stream);
        }
        ...
        if (n == 0) {
            ...
            void* kernelargs[] = {(void*)&n, (void*)&d_p, (void*)&d_t, (void*)&d_pdott_dev};
            dim3 blockDim(blocksize, 1, 1);
            dim3 gridDim(mingridsize, 1, 1);
            nvshmemx_collective_launch((void*)ddot_allreduce_fused, gridDim, blockDim, kernelargs, sharedmemsize,
                                       stream);
            cudaStreamSynchronize(stream);
        }
...
int main(int argc, char* argv[]) {
    ...
    if (rank == 0) {
        printf("MPI Initialized: size = %d\n", size);
        printf("NVSHMEM Initialized via MPI: npes = %d\n", npes);
    }
    ...
    if (rank != mype || size != npes) {
        ...
        MPI_Barrier(MPI_COMM_WORLD);
        if (rank == 0) printf("MPI Ranks and NVSHMEM PEs consistency verified.\n");
    }
