...
int cg_solver_init(CGSolverData* cg, int local_rows, int local_nnz, const int* h_A_rowptr,
    ...
    if (local_rows > 0) {
        ...
        GPU_RT_CALL(cudaMalloc((void**)&cg->d_p, local_rows * sizeof(double)));  // CHANGED: double
...
int nccl_allgatherv(const void* sendbuf, int sendcount, void* recvbuf,
                    ...
                    ncclDataType_t datatype, size_t element_size, ncclComm_t comm, cudaStream_t stream) {
