...
__global__ void comm_kernel(real *__restrict__ const a_new, real *__restrict__ const a, int nx,
                            uint64_t *__restrict__ signal_buf, size_t window_size, uint64_t i, int neighbour,
                            uniconn::Communicator<> *comm) {
    cg::grid_group grid = cg::this_grid();
    if (neighbour == 1) {
        uniconn::Coordinator<>::Post<uniconn::ThreadGroup::BLOCK>(a, a_new, nx, signal_buf + blockIdx.x, i + 1,
                                                                  neighbour, comm);
        grid.sync();
        if (blockIdx.x == 0 && threadIdx.x == 0) {
            uniconn::Coordinator<>::Acknowledge<uniconn::ThreadGroup::THREAD>(a_new, size_t(1), signal_buf, i + 1,
                                                                              neighbour, comm);
        }

    } else if (neighbour == 0) {
        if (threadIdx.x == 0) {
            uniconn::Coordinator<>::Acknowledge<uniconn::ThreadGroup::THREAD>(a_new, nx, signal_buf + blockIdx.x, i + 1,
                                                                              neighbour, comm);
        }
        grid.sync();
        if (blockIdx.x == 0) {
            uniconn::Coordinator<>::Post<uniconn::ThreadGroup::BLOCK>(a, a_new, size_t(1), signal_buf, i + 1, neighbour,
                                                                      comm);
        }
    }
}
...
int main(int argc, char *argv[]) {
    ...
    long long unsigned int required_symmetric_heap_size =
        (2 * max_msg_size * sizeof(real) + window_size * sizeof(uint64_t)) *
    ...
    uint64_t *signal_buff = uniconn::Memory<>::Alloc<uint64_t>(window_size);

    UncGpuStream_t stream = 0;
    ...
    GPU_RT_CALL(UncGpuMemsetAsync(signal_buff, 0, window_size * sizeof(uint64_t), stream));

    const int neighbour = (rank + 1) % size;
    int message_size = 1;
    uint64_t i = 0;

    void *kernelArgs[] = {(void *)&recv_buff,   (void *)&send_buff, (void *)&message_size, (void *)&signal_buff,
                          (void *)&window_size, (void *)&i,         (void *)&neighbour,    (void *)&comm_d};

    uniconn::Coordinator<> send_coordinator(stream);

    send_coordinator.bindKernel<uniconn::LaunchMode::FullDevice>((void *)comm_kernel, dim3(window_size), dim3(1024), 0,
                                                                 kernelArgs);

    uniconn::Coordinator<> recv_coordinator(stream);

    recv_coordinator.bindKernel<uniconn::LaunchMode::FullDevice>((void *)comm_kernel, dim3(window_size), dim3(1), 0,
                                                                 kernelArgs);

    int iter_max = BW_LOOP_SMALL;
    uint64_t warmup = BW_SKIP_SMALL;
    for (message_size = 1; message_size <= max_msg_size; message_size *= 2) {
        ...
        if (rank == 0) {
            ...
            for (i = 0; i < warmup + iter_max; i++) {
                ...
                send_coordinator.CommStart();
                for (int j = 0; j < window_size; ++j) {
                    send_coordinator.Post(send_buff, recv_buff, message_size, signal_buff + j, i + 1, neighbour, &comm);
                }
                send_coordinator.CommEnd();
                send_coordinator.Acknowledge(recv_buff, 1, signal_buff, i + 1, neighbour, &comm);
            }
            ...
            printf("bandwidth, unc, %d, %f\n", message_size,
                   ((message_size / 1e6 * window_size * iter_max) / (end-start)));

        } else if (rank == 1)  // process 1
        {
            for (i = 0; i < iter_max + warmup; i++) {
                recv_coordinator.LaunchKernel();
                recv_coordinator.CommStart();
                for (int j = 0; j < window_size; ++j) {
                    recv_coordinator.Acknowledge(recv_buff, message_size, signal_buff + j, i + 1, neighbour, &comm);
                }
                recv_coordinator.CommEnd();
                recv_coordinator.Post(send_buff, recv_buff, 1, signal_buff, i + 1, neighbour, &comm);
            }
        }
    }
