...
__global__ void comm_kernel(real *__restrict__ const a_new, const real *__restrict__ const a, const int nx,
                            uint64_t *__restrict__ const signal_buf, const int window_size, const uint64_t i,
                            const int neighbour) {
    cg::grid_group grid = cg::this_grid();
    if (neighbour == 1) {
        nvshmemx_putmem_signal_nbi_block(a_new, a, nx * sizeof(real), signal_buf + blockIdx.x, 1, NVSHMEM_SIGNAL_ADD,
                                         neighbour);
        grid.sync();
        ...
        if (threadIdx.x == 0) {
            nvshmem_signal_wait_until(signal_buf + blockIdx.x, NVSHMEM_CMP_GE, i + 1);
        }
        grid.sync();
        if (blockIdx.x == 0) {
            nvshmemx_putmem_signal_nbi_block(a_new, a, 1 * sizeof(real), signal_buf, 1, NVSHMEM_SIGNAL_ADD, neighbour);
        }
    }
}
...
int main(int argc, char *argv[]) {
    ...
    uint64_t *signal_buff = (uint64_t *)nvshmem_malloc(window_size * sizeof(uint64_t));
    ...
    void *kernelArgs[] = {(void *)&recv_buff,   (void *)&send_buff, (void *)&message_size, (void *)&signal_buff,
                          (void *)&window_size, (void *)&i,         (void *)&neighbour};
    ...
    for (message_size = 1; message_size <= max_msg_size; message_size *= 2) {
        ...
        GPU_RT_CALL(UncGpuMemsetAsync(signal_buff, 0, window_size * sizeof(uint64_t), nvshmem_stream));
        ...
        if (mype == 0) {
            ...
            for (i = 0; i < warmup + iter_max; i++) {
                ...
                GPU_RT_CALL((UncGpuError_t)nvshmemx_collective_launch((const void *)comm_kernel, dim3(window_size),
                                                                      dim3(1024), kernelArgs, 0, nvshmem_stream));
            }
            ...
            printf("bandwidth, nvshmem_d, %d, %f\n", message_size,
                   ((message_size / 1e6 * window_size * iter_max) / (end - start)));

        } else if (mype == 1)  // process 1
        {
            for (i = 0; i < iter_max + warmup; i++) {
                GPU_RT_CALL((UncGpuError_t)nvshmemx_collective_launch((void *)comm_kernel, dim3(window_size), dim3(1),
                                                                      kernelArgs, 0, nvshmem_stream));
            }
        }
    }
