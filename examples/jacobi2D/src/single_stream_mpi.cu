...
int main(int argc, char *argv[]) {
    ...
    GPU_RT_CALL(UncGpuMalloc(&a_comm, nx * 2 * sizeof(real)));
    GPU_RT_CALL(UncGpuMalloc(&a_new_comm, nx * 2 * sizeof(real)));
    ...
    for (uint64_t iter = 0; iter < iter_max + warmup; ++iter) {
        ...
        jacobi_kernel<<<dim_grid, dim3(dim_block_x, dim_block_y, 1), 0, compute_stream>>>(a_new, a, iy_start, iy_end,
                                                                                          nx, a_comm);
        GPU_RT_CALL(UncGpuStreamSynchronize(compute_stream));
        MPI_CALL(MPI_Isend(a_new + iy_start * nx, nx, MPI_REAL_TYPE, top_pe, 0, MPI_COMM_WORLD, &(req[0])));
        MPI_CALL(MPI_Isend(a_new + (iy_end - 1) * nx, nx, MPI_REAL_TYPE, bottom_pe, 0, MPI_COMM_WORLD, &(req[1])));
        MPI_CALL(MPI_Irecv(a_new_comm, nx, MPI_REAL_TYPE, bottom_pe, 0, MPI_COMM_WORLD, &(req[2])));
        MPI_CALL(MPI_Irecv(a_new_comm + nx, nx, MPI_REAL_TYPE, top_pe, 0, MPI_COMM_WORLD, &(req[3])));
        MPI_CALL(MPI_Waitall(4, req, MPI_STATUSES_IGNORE));
