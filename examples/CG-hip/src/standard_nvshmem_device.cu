...
int cg_solver_solve(CGSolverData* cg,
    ...
    if (cg->comm_size > 1) {
        ...
        GPU_RT_CALL(
            hipMemcpyAsync(recvcounts_d, recvcounts, cg->comm_size * sizeof(int), hipMemcpyHostToDevice, stream));
        GPU_RT_CALL(hipMemcpyAsync(displs_d, displs, cg->comm_size * sizeof(int), hipMemcpyHostToDevice, stream));
    }
    ...
    if (n == 0) {
        GPU_RT_CALL(hipMemset(d_rnrm2sqr_dev, 0, sizeof(double)));  // CHANGED: double
    } else {
        static int mingridsize = 0, blocksize = 0, sharedmemsize = 0;
        if (mingridsize == 0 && blocksize == 0) {
