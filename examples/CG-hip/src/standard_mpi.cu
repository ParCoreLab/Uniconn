...
void cg_solver_free(CGSolverData* cg)
{
    ...
    if (cg->matA_descr)
        CHECK_HIPSPARSE(hipsparseDestroySpMat(cg->matA_descr));
    cg->matA_descr = NULL;
    if (cg->vecr_descr)
        CHECK_HIPSPARSE(hipsparseDestroyDnVec(cg->vecr_descr));
    cg->vecr_descr = NULL;
    if (cg->vecp_descr)
        CHECK_HIPSPARSE(hipsparseDestroyDnVec(cg->vecp_descr));
    cg->vecp_descr = NULL;
    if (cg->vect_descr)
        CHECK_HIPSPARSE(hipsparseDestroyDnVec(cg->vect_descr));
    cg->vect_descr = NULL;
    ...
    hipsparseSpMVAlg_t alg)
{
    ...
    if (local_rows > 0)
    {
        ...
        GPU_RT_CALL(
            hipMemset(cg->d_r, 0, local_rows * sizeof(double))); // CHANGED: double
        GPU_RT_CALL(
            hipMemset(cg->d_p, 0, local_rows * sizeof(double))); // CHANGED: double
        GPU_RT_CALL(
            hipMemset(cg->d_t, 0, local_rows * sizeof(double))); // CHANGED: double

        // --- Allocate and copy sparse matrix (local part) to device ---
        GPU_RT_CALL(hipMalloc(
            (void**) &cg->d_A_rowptr,
            (local_rows + 1) * sizeof(int)));
        GPU_RT_CALL(hipMemcpy(
            cg->d_A_rowptr, h_A_rowptr,
            (local_rows + 1) * sizeof(int), hipMemcpyHostToDevice));
        GPU_RT_CALL(hipMalloc(
            (void**) &cg->d_A_colidx,
        ...
        CHECK_HIPSPARSE(hipsparseCreateCsr(
        ...
        CHECK_HIPSPARSE(hipsparseCreateDnVec(
    ...
    GPU_RT_CALL(hipMemcpy(
        cg->d_one, &one, sizeof(double), hipMemcpyHostToDevice)); // CHANGED: double
    ...
    GPU_RT_CALL(hipMemcpy(
        cg->d_zero, &zero, sizeof(double), hipMemcpyHostToDevice)); // CHANGED: double
    ...
    GPU_RT_CALL(
        hipMalloc((void**) &cg->d_rnrm2sqr_prev, sizeof(double))); // CHANGED: double
    ...
    hipsparseDnVecDescr_t tmp_local_target_descr =
        cg->vect_descr;
    double* tmp_d_local_target = cg->d_t; // CHANGED: double

    GPU_RT_CALL(
        hipMalloc((void**)&tmp_d_global_vec, cg->N_global * sizeof(double))); // CHANGED: double
    CHECK_HIPSPARSE(hipsparseCreateDnVec(
    ...
    CHECK_HIPSPARSE(hipsparseSpMV_bufferSize(
        hipsparse, HIPSPARSE_OPERATION_NON_TRANSPOSE, cg->d_one,
        tmp_mat_descr, tmp_global_vec_descr, cg->d_zero,
        tmp_local_target_descr, HIP_R_64F, alg, &bufferSize)); // CHANGED: HIP_R_64F
