...
for mtx in $Dataset_Dir/*.mtx; do
    ...
    while [ $iter -lt $iter_max ]; do
        srun --cpu-bind=${CPU_BIND} -n 16 -G 16 standard_mpi $mtx
        srun --cpu-bind=${CPU_BIND} -n 16 -G 16 standard_uniconn_mpi $mtx
        
        srun --cpu-bind=${CPU_BIND} -n 16 -G 16 standard_gpuccl $mtx
        srun --cpu-bind=${CPU_BIND} -n 16 -G 16 standard_uniconn_gpuccl $mtx
        iter=$((iter + 1))
    done
done
